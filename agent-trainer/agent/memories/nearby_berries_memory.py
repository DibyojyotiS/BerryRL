import numpy as np
from numba import njit
from numba.typed import Dict
from .base_class import MemoryBase


@njit
def njitUpdateMinTree(index, berryScore, minTree):
    memorySize = len(minTree)//2
    idx = index + memorySize
    minTree[idx][0] = berryScore
    minTree[idx][1] = index
    while idx > 1:
        idx//=2
        if minTree[2*idx][0] > minTree[2*idx+1][0]:
            minTree[idx] = minTree[2*idx+1]
        else:
            minTree[idx] = minTree[2*idx]


@njit
def njitRemoveBerriesOutOfRange(
    agentPosXY, minDistPopThXY, maxDistPopThXY, memory, minTree, hashMap
):
    # remove any berry whose distance from agent is is out of range
    # returns the number of berries removed
    relativeXY = np.abs(memory[:,:-1] - np.asarray(agentPosXY))
    indices = np.nonzero(
        (relativeXY[:,0] > maxDistPopThXY[0]) 
        | (relativeXY[:,1] > maxDistPopThXY[1])
        | ((relativeXY[:,0] < minDistPopThXY[0]) 
        & (relativeXY[:,1] < minDistPopThXY[1]))
    )[0]
    nRemoved = 0
    for index in indices:
        if memory[index][2] == -1.0: continue
        # TODO getting key error here sometimes
        hashMap.pop((int(memory[index][0]), int(memory[index][1]), int(memory[index][2])), -1)
        memory[index][0] = memory[index][1] = memory[index][2] = -1
        njitUpdateMinTree(index, -1, minTree)
        nRemoved += 1
    return nRemoved

@njit
def njitAddBerry(
    berryScore, berryRelPosXY, berrySize, agentPosXY, minDistPopThXY, 
    maxDistPopThXY, memory, minTree, hashMap
):
    # add the new berry to the memory if it's score is greater than the minimum score
    # returns 0/1, 1 if the berry as inserted; 0 if berry was discarded/updated
    # berryRelPosXY is the relative position of the berry from the agent
    absBerryPosX = abs(berryRelPosXY[0])
    absBerryPosY = abs(berryRelPosXY[1])
    if (
        ((absBerryPosX >= minDistPopThXY[0]) 
        | (absBerryPosY >= minDistPopThXY[1]))
        & (absBerryPosX <= maxDistPopThXY[0]) 
        & (absBerryPosY <= maxDistPopThXY[1])
    ):
        min_berry_score = minTree[1][0]
        roundedBerryPosX = round(berryRelPosXY[0] + agentPosXY[0], 2) # absolute x coordinate of berry
        roundedBerryPosY = round(berryRelPosXY[1] + agentPosXY[1], 2) # absolute y coordinate of berry
        posIx = (int(roundedBerryPosX), int(roundedBerryPosY), int(berrySize))

        # if the berry already exists in the memory then simply update the score and exits
        currentIdx = hashMap.get(posIx, -1)
        if currentIdx != -1:
            njitUpdateMinTree(currentIdx, berryScore, minTree)
            return 0 # berry was updated

        # insert a new berry only if it's score is larger than the minimum score in memory
        if (berryScore > min_berry_score):
            min_index = int(minTree[1][1])
            memory[min_index][0] = roundedBerryPosX
            memory[min_index][1] = roundedBerryPosY
            memory[min_index][2] = berrySize
            hashMap[posIx] = min_index
            njitUpdateMinTree(min_index, berryScore, minTree)
            return 1 # berry was inserted

    return 0 # berry was ignored


@njit
def njitBulkAddBerries(
    listOfBerries, listOfberryScores, agentPosXY, minDistPopThXY, 
    maxDistPopThXY, memory, minTree, hashMap
):
    if len(listOfBerries) == 0: return 0
    numInserted = 0
    for i in range(len(listOfBerries)):
        berryScore = listOfberryScores[i]
        berryRelPosXY = listOfBerries[i, :2]
        berrySize = listOfBerries[i, 2]
        numInserted += njitAddBerry(
            berryScore, berryRelPosXY, berrySize, agentPosXY, minDistPopThXY, 
            maxDistPopThXY, memory, minTree, hashMap
        )
    return numInserted


@njit
def njitGetMemorizedBerriesRelativePosition(agentPosXY, memory=np.array([[]])):
    indices = np.nonzero(memory[:,-1] > 0)[0]
    returnArray = np.zeros((len(indices), 3))
    for i, index in enumerate(indices):
        returnArray[i][0] = memory[index][0] - agentPosXY[0]
        returnArray[i][1] = memory[index][1] - agentPosXY[1]
        returnArray[i][2] = memory[index][2]
    return returnArray


class NearbyBerriesMemory(MemoryBase):
    def __init__(self, minDistPopThXY, maxDistPopThXY, memorySize) -> None:
        self.minDistPopThXY = minDistPopThXY
        self.maxDistPopThXY = maxDistPopThXY
        self.memorySize = memorySize
        self.reset()

    def insertOrUpdate(self, berryScore, berryRelPosXY, berrySize, agentPosXY):
        """ berryRelPosXY is the relative position of the berry with 
        respect to the agent """
        nRemoved = self._removeBerriesOutOfRange(agentPosXY)
        nInserted = njitAddBerry(
            berryScore, berryRelPosXY, berrySize, agentPosXY, 
            self.minDistPopThXY, self.maxDistPopThXY, self.memory, 
            self.minTree, self.hashMap
        )
        self._updateSize(nInserted, nRemoved)
        self._updateStats()

    def bulkInsertOrUpdate(self, listOfBerries, listOfberryScores, agentPosXY):
        """ listOfBerries is a list of [x,y,berry-size] where x,y are 
        relative postion of the berry with rspect to the agent """
        nRemoved = self._removeBerriesOutOfRange(agentPosXY)
        nInserted = njitBulkAddBerries(
            listOfBerries, listOfberryScores, agentPosXY, self.minDistPopThXY, 
            self.maxDistPopThXY, self.memory, self.minTree, self.hashMap
        )
        self._updateSize(nInserted, nRemoved)
        self._updateStats()

    def getMemoryBerriesAsRelativePos(self, agentPosXY):
        # this call to remove berries is added because it can happen that
        # the agent breakes out of random exploration without updating the 
        # memories. Thus the returned berry memory would be incorrect
        # without this check.
        nRemoved = self._removeBerriesOutOfRange(agentPosXY)
        self._updateSize(0, nRemoved)
        self._updateStats()
        return njitGetMemorizedBerriesRelativePosition(agentPosXY, self.memory)

    def reset(self):
        self._initMemory()
        self._initStats()

    def size(self):
        return self.nBerriesInMemory

    def getStats(self):
        return {
            "c": self.maxBerriesInMemoryStat
        }

    def _updateStats(self):
        self.maxBerriesInMemoryStat = max(self.maxBerriesInMemoryStat, self.size())

    def _initStats(self):
        self.maxBerriesInMemoryStat = 0

    def _initMemory(self):
        self.memory = np.zeros((self.memorySize, 3), dtype=np.float32) # x, y, berry_size
        self.minTree = np.zeros((2*self.memorySize, 2)) # berry_score, memory-index
        self.hashMap = Dict()
        self.memory[:] = -1
        self.hashMap[(-1,-1,-1)] = 1
        self.nBerriesInMemory = 0 # size

        # init min-tree
        for i in range(self.memorySize):
            njitUpdateMinTree(i, -1, self.minTree)

        # for optimization, call njitRemoveBerriesOutOfRange atmost once 
        # if the last called position is not changed.
        self.last_pruned_pos = (float('inf'),float('inf'))

    def _removeBerriesOutOfRange(self, agentPosXY):
        if self.last_pruned_pos == agentPosXY: return 0 # no berries removed
        self.last_pruned_pos = agentPosXY
        return njitRemoveBerriesOutOfRange(
            agentPosXY, self.minDistPopThXY, self.maxDistPopThXY, 
            self.memory, self.minTree, self.hashMap
        )
        

    def _updateSize(self, nInserted, nRemoved):
        self.nBerriesInMemory += nInserted - nRemoved


if __name__ == "__main__":
    from time import time_ns

    def temp_berry_worth(sizes:np.ndarray, distances:np.ndarray, 
                REWARD_RATE, DRAIN_RATE, HALFDIAGOBS, WORTH_OFFSET=0,
                min_berry_size=10, max_berry_size=40):
        rr, dr = REWARD_RATE, DRAIN_RATE
        worth = rr * sizes - dr * distances
        min_worth = rr * min_berry_size - dr * HALFDIAGOBS
        max_worth = rr * max_berry_size
        worth = (worth - min_worth)/(max_worth - min_worth)
        worth = np.clip(worth, a_min=0,a_max=None)
        worth = (worth + WORTH_OFFSET)/(1 + WORTH_OFFSET)
        return worth

    def setup():
        xy = np.asfarray(np.random.randint(-1300, 1300, size=(80,2)))
        sizes = np.asfarray(np.random.randint(1,5, size=80)*10)
        berries = np.column_stack([xy[:,0], xy[:,1], sizes])
        distances = np.linalg.norm(xy, axis=1)
        worths = temp_berry_worth(
            sizes, distances, REWARD_RATE=1e-4, DRAIN_RATE=1/(2*120*400), 
            HALFDIAGOBS = 0.5*(1920**2 + 1080**2)**0.5
        )
        return berries, worths

    mem = NearbyBerriesMemory((10,10), (150,150), 50)
    berries, worths = setup()
    mem.insertOrUpdate(0.2, (10,10), 20, (0,0))
    mem.bulkInsertOrUpdate(berries, worths, (0,0))
    mem.reset()

    times = []
    for i in range(10000):
        berries, worths = setup()
        startT = time_ns()
        mem.bulkInsertOrUpdate(berries, worths, (0,0))
        times.append(time_ns() - startT)
    print("average time bulkUpdate:", np.average(times), "ns")