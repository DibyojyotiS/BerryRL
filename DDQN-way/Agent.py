...
class Agent():
    ...
    def env_step_wrapper(self, berryField:BerryFieldEnv, **kwargs):
    ...
    def _update_memories(self, info, avg_worth):
        ...
        x = x//(self.berryField.FIELD_SIZE[0]//mem_x)
        x = x//(self.berryField.FIELD_SIZE[1]//mem_y)
        index = int(x + y*mem_x)
    ...
    def _computeState(self, raw_observation, info, reward, done) -> np.ndarray:
        ...
        state = np.concatenate([*sectorized_states, 
                edge_dist, patch_relative, [total_juice], [self.time_memory], rel_p])

        # apply persistence
        if self.prev_state is not None: 
            state = self.prev_state * self.persistence + (1-self.persistence)*state
    ...
    def getNet(self, TORCH_DEVICE, debug=False,
        ...
        class net(nn.Module):
            def __init__(self):
                ...
                self.m_part = (4*num_sectors,4*num_sectors+9)
            ...
            def forward(self, input:Tensor):
                ...
                for layer in self.final_stage: add = layer(add)

                value = self.valueL(add)
                advs = self.actadvs(add)
